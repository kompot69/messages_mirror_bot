...
action_join_in_chat='ℹ️ Присоеденяется к чату'
action_left_from_chat='ℹ️ Покидает чат'
action_chat_invite_user='ℹ️ Добавил(а) пользователя {added_user} в чат'
action_chat_invite_user_by_link='ℹ️ Присоеденяется к чату по ссылке'
action_chat_kick_user='ℹ️ Удалил(а) пользователя {kicked_user} из чата'
action_chat_title_update='ℹ️ Изменил(а) название чата на "{new_chat_name}"'
action_chat_photo_update='ℹ️ Изменил(а) аватар чата'
action_chat_without_owner_admin='ℹ️ Владелец чата вышел из чата'
action_chat_pin_message='ℹ️ Закрепляет сообщение'
action_chat_unpin_message='ℹ️ Открепляет сообщение'

notificate_attachments_error='ℹ️ Не удалось переслать вложения в {service_to}: '
...
chat_has_no_connected_chats='Проверьте настройки подключенных чатов командой /mirror'
...
admin_text='🔢 Версия: {bot_version}\n💾Использовано ОП: {ram_usage}\n*️⃣ Макс.длина сообщения: {message_max_len}'
def cmd_mirror_status(service0, service0_chat_id, other_services:dict, service0_message_access=True, admin_text=False): # other_services service: [cutag, chat_id, chat_name, connected_mutually]
    ...
    if config.attachments_forward: answer += f'\n\nℹ️ Доступна пересылка вложений до {config.attachments_max_size_mb} МБ'
